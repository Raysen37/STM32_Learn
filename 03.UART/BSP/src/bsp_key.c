...
uint8_t Read_Key1_Level(void) {
    return HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_15); // 读取PB15
}

uint8_t Read_Key2_Level(void) {
    return HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0); // 读取PA0
}
...
//        PRINT_ERR("struct button is null!");
    ...
    memset(btn, 0, sizeof(Button_t));  // 清除结构体信息，建议用户在之前清除
