...
{
	I2C_Start();				//I2C起始
	I2C_WriteByte(0x78);		//发送OLED的I2C从机地址
	I2C_WriteByte(0x00);		//控制字节，给0x00，表示即将写命令
	I2C_WriteByte(Command);		//写入指定的命令
	I2C_Stop();				//I2C终止
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	I2C_Start();				//I2C起始
	I2C_WriteByte(0x78);		//发送OLED的I2C从机地址
	I2C_WriteByte(0x40);		//控制字节，给0x40，表示即将写数据
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		I2C_WriteByte(Data[i]);	//依次发送Data的每一个数据
	}
	I2C_Stop();				//I2C终止
}
